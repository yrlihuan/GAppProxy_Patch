...
class LocalProxyHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def do_METHOD(self):
        ...
        resp = self.get_response_from_gae(params)
        if not resp:
            return
    ...
    def get_response_from_gae(self, params):
        ...
        if local_proxy:
            ...
            try:
                ...
                if local_proxy == "" and not google_proxy:
                    shallWeNeedGoogleProxy()
                self.connection.close()
                return
        else:
            # if we need to go through google.cn,
            # send the request via socket directly
            try:
                sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
